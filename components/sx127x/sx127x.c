 ...
 */
void 
sx127x_write_reg(int reg, int val)
{
 ...
 */
void
sx127x_write_reg_buffer(int reg, uint8_t *val, int len)
{
   uint8_t *out;
 ...
 */
void
sx127x_read_reg_buffer(int reg, uint8_t *val, int len)
{
   uint8_t *out;
   uint8_t *in;
   out = (uint8_t *)malloc(len+1);
...
/**
 * Perform physical reset on the sx127x chip
 */
void 
sx127x_reset(void)
{
 ...
 */
void 
sx127x_explicit_header_mode(void)
{
   __implicit = 0;
 ...
 */
void 
sx127x_implicit_header_mode(int size)
{
   __implicit = 1;
   sx127x_write_reg(REG_MODEM_CONFIG_1, sx127x_read_reg(REG_MODEM_CONFIG_1) | 0x01);
 ...
 */
void 
sx127x_set_frequency(long frequency)
{
 ...
 */
void 
sx127x_set_spreading_factor(int sf)
{
   if (sf < 6) sf = 6;
   else if (sf > 12) sf = 12;

   if (sf == 6) {
      sx127x_write_reg(REG_DETECTION_OPTIMIZE, 0xc5);
      sx127x_write_reg(REG_DETECTION_THRESHOLD, 0x0c);
   } else {
      sx127x_write_reg(REG_DETECTION_OPTIMIZE, 0xc3);
      sx127x_write_reg(REG_DETECTION_THRESHOLD, 0x0a);
   }

   sx127x_write_reg(REG_MODEM_CONFIG_2, (sx127x_read_reg(REG_MODEM_CONFIG_2) & 0x0f) | ((sf << 4) & 0xf0));
}
 ...
 */
int 
sx127x_get_spreading_factor(void)
{
   return (sx127x_read_reg(REG_MODEM_CONFIG_2) >> 4);
}
 ...
 */
void 
sx127x_set_dio_mapping(int dio, int mode)
{
   if (dio < 4) {
      int _mode = sx127x_read_reg(REG_DIO_MAPPING_1);
      if (dio == 0) {
         _mode = _mode & 0x3F;
         _mode = _mode | (mode << 6);
      } else if (dio == 1) {
      ...
      sx127x_write_reg(REG_DIO_MAPPING_1, _mode);
      ESP_LOGD(TAG, "REG_DIO_MAPPING_1=0x%02x", _mode);
   } else if (dio < 6) {
      int _mode = sx127x_read_reg(REG_DIO_MAPPING_2);
      if (dio == 4) {
         _mode = _mode & 0x3F;
         _mode = _mode | (mode << 6);
      } else if (dio == 5) {
      ...
      sx127x_write_reg(REG_DIO_MAPPING_2, _mode);
   }
}
 ...
 */
int 
sx127x_get_dio_mapping(int dio)
{
   if (dio < 4) {
      int _mode = sx127x_read_reg(REG_DIO_MAPPING_1);
      ESP_LOGD(TAG, "REG_DIO_MAPPING_1=0x%02x", _mode);
 ...
 */
void 
sx127x_set_bandwidth(int sbw)
{
   if (sbw < 10) {
      sx127x_write_reg(REG_MODEM_CONFIG_1, (sx127x_read_reg(REG_MODEM_CONFIG_1) & 0x0f) | (sbw << 4));
   }
}
 ...
 */
int 
sx127x_get_bandwidth(void)
{
   //int bw;
   //bw = sx127x_read_reg(REG_MODEM_CONFIG_1) & 0xf0;
   //ESP_LOGD(TAG, "bw=0x%02x", bw);
   //bw = bw >> 4;
   //return bw;
   return ((sx127x_read_reg(REG_MODEM_CONFIG_1) & 0xf0) >> 4);
}
 ...
 */ 
void 
sx127x_set_coding_rate(int denominator)
{
   if (denominator < 5) denominator = 5;
   else if (denominator > 8) denominator = 8;

   int cr = denominator - 4;
   sx127x_write_reg(REG_MODEM_CONFIG_1, (sx127x_read_reg(REG_MODEM_CONFIG_1) & 0xf1) | (cr << 1));
}
 ...
 */ 
int 
sx127x_get_coding_rate(void)
{
   return ((sx127x_read_reg(REG_MODEM_CONFIG_1) & 0x0E) >> 1);
}
 ...
 */
void 
sx127x_set_preamble_length(long length)
{
   sx127x_write_reg(REG_PREAMBLE_MSB, (uint8_t)(length >> 8));
   sx127x_write_reg(REG_PREAMBLE_LSB, (uint8_t)(length >> 0));
}
 ...
 */
long
sx127x_get_preamble_length(void)
{
   long preamble;
   preamble = sx127x_read_reg(REG_PREAMBLE_MSB) << 8;
   preamble = preamble + sx127x_read_reg(REG_PREAMBLE_LSB);
   return preamble;
}
 ...
 */
void 
sx127x_set_sync_word(int sw)
{
   sx127x_write_reg(REG_SYNC_WORD, sw);
}
 ...
 */
void 
sx127x_disable_crc(void)
{
   sx127x_write_reg(REG_MODEM_CONFIG_2, sx127x_read_reg(REG_MODEM_CONFIG_2) & 0xfb);
}
...
sx127x_init(void)
{
    ...
    */
   sx127x_sleep();
   sx127x_write_reg(REG_FIFO_RX_BASE_ADDR, 0);
   sx127x_write_reg(REG_FIFO_TX_BASE_ADDR, 0);
   sx127x_write_reg(REG_LNA, sx127x_read_reg(REG_LNA) | 0x03);
   sx127x_write_reg(REG_MODEM_CONFIG_3, 0x04);
 ...
 */
void 
sx127x_send_packet(uint8_t *buf, int size)
{
   /*
...
#if BUFFER_IO
   sx127x_write_reg_buffer(REG_FIFO, buf, size);
#else
   for(int i=0; i<size; i++) 
      sx127x_write_reg(REG_FIFO, *buf++);
#endif
 ...
 */
int 
sx127x_receive_packet(uint8_t *buf, int size)
{
   int len = 0;
    ...
    * Find packet size.
    */
   if (__implicit) len = sx127x_read_reg(REG_PAYLOAD_LENGTH);
   else len = sx127x_read_reg(REG_RX_NB_BYTES);
...
#if BUFFER_IO
   sx127x_read_reg_buffer(REG_FIFO, buf, len);
#else
   for(int i=0; i<len; i++) 
 ...
 */
int
sx127x_received(void)
{
   if(sx127x_read_reg(REG_IRQ_FLAGS) & IRQ_RX_DONE_MASK) return 1;
   return 0;
}
...
sx127x_get_irq(void)
{
   return (sx127x_read_reg(REG_IRQ_FLAGS));
}
 ...
 */
float 
sx127x_packet_snr(void)
{
   return ((int8_t)sx127x_read_reg(REG_PKT_SNR_VALUE)) * 0.25;
}
...
{
   sx127x_sleep();
//   close(__spi);  FIXME: end hardware features after sx127x_close
//   close(__cs);
//   close(__rst);
//   __spi = -1;
//   __cs = -1;
...
void sx127x_config(sx127xConf_t sx127xConf){
   sx127x_set_frequency(sx127xConf.frequency);
   sx127x_enable_crc();
   sx127x_set_coding_rate(sx127xConf.codingRate);
   sx127x_set_bandwidth(sx127xConf.bandwith);
   sx127x_set_spreading_factor(sx127xConf.spreadingFactor);
}